...
export async function ask(userText: string): Promise<string> {
  ...
  try {
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      // Beta endpoint for server-side refusal fallbacks (Fable 5 -> Opus 4.8).
      // Thinking is always on for claude-fable-5, so no `thinking` param is sent.
      // Params cast: fallbacks/output_config typings can lag the API — shape is per docs.
      ...
      const stream: any = anthropic.beta.messages.stream({
        ...
        messages: history,
        tools,
        output_config: { effort: settings.anthropic.effort },
